...
/**
 * Modal de gerenciamento em lote para admins.
 * Busca todas as empresas ativas e permite adicionar/remover impostos ou obrigações.
 */
export default function AdminBatchModal({ onClose, onSuccess, initialType = "tax" }) {
  ...
  useEffect(() => {
    (async () => {
      setDataLoading(true);
      try {
        const [taxRes, oblRes, compRes] = await Promise.all([
          api.get("/tax/all"),
        ...
        setCompanies((compRes.data || []).filter((c) => !c.isArchived && c.status === "ATIVA"));
      } catch {
  ...
  const toggleCompany   = (id) => setSelectedIds((prev) => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-dark-card rounded-2xl shadow-xl w-full max-w-4xl max-h-[92vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-dark-border">
          <h2 className="text-base font-semibold text-light-text dark:text-dark-text">Gerenciar em Lote</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="flex border border-gray-200 dark:border-dark-border rounded-xl overflow-hidden">
                {[["add","Adicionar"],["remove","Remover"]].map(([val, lbl], i) => (
                  <button key={val} type="button" onClick={() => setAction(val)}
                    className={`flex-1 py-1.5 text-xs font-medium transition-colors ${action === val ? "bg-primary-500 text-white" : "bg-white dark:bg-dark-surface text-gray-600 hover:bg-gray-50"} ${i > 0 ? "border-l border-gray-200 dark:border-dark-border" : ""}`}>
                    {lbl}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex border border-gray-200 dark:border-dark-border rounded-xl overflow-hidden">
                {[["tax","Imposto"],["obligation","Obrigação"]].map(([val, lbl], i) => (
                  <button key={val} type="button" onClick={() => { setType(val); setItemId(""); }}
                    className={`flex-1 py-1.5 text-xs font-medium transition-colors ${type === val ? "bg-primary-500 text-white" : "bg-white dark:bg-dark-surface text-gray-600 hover:bg-gray-50"} ${i > 0 ? "border-l border-gray-200 dark:border-dark-border" : ""}`}>
                    {lbl}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="label-base mb-2">{type === "tax" ? "Imposto" : "Obrigação"} a {action === "add" ? "adicionar" : "remover"}</label>
              <select value={itemId} onChange={(e) => setItemId(e.target.value)} className="input-base w-full">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="label-base">Empresas ({selectedIds.size} selecionada{selectedIds.size !== 1 ? "s" : ""} de {filteredCompanies.length})</span>
                  ...
                  <table className="min-w-full">
                    <thead>
                      <tr>
                        ...
                        <th className={thClass} onClick={() => handleSort("branchNumber")}>
                          <span className="inline-flex items-center">Filial <SortIcon field="branchNumber" sortField={sortField} sortDir={sortDir} /></span>
                        </th>
                          ...
                          return (
                            ...
                            >
                              <td className="table-cell text-center !px-2">
                                <input type="checkbox" checked={checked} onChange={() => toggleCompany(company.id)}
                              ...
                              <td className="table-cell text-xs text-center">{company.branchNumber || "–"}</td>
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-dark-border flex justify-end gap-3">
          ...
          >
            ...
            {action === "add" ? "Adicionar" : "Remover"} para {selectedIds.size} empresa{selectedIds.size !== 1 ? "s" : ""}
          </button>
        </div>
      </div>
    </div>
