...
const BonusPage = () => {
  ...
  const handleRecalculate = async () => {
    if (
      !confirm(
        "Tem certeza que deseja recalcular todos os bônus? Os dados atuais serão substituídos."
      )
    ) {
      return;
    }
      ...
      <div className="w-full px-8 py-8 text-gray-800 dark:text-dark-text">
        ...
        <div className="bg-white dark:bg-dark-card p-6 rounded-lg shadow-md mb-6">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <FiDollarSign /> Fatores de Cálculo
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            ...
            <div>
              <label
                htmlFor="fiscal_valor_base_c"
                className="block text-sm font-medium mb-1"
              >
                Fiscal: Valor do Bônus Básico (C)
              </label>
              <input
                type="number"
                step="0.01"
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2 bg-accent-green text-white font-semibold rounded-lg shadow-md hover:bg-green-700 transition-colors"
            >
        ...
        <div className="bg-white dark:bg-dark-card p-6 rounded-lg shadow-md">
          ...
          {view === "dp" && (
            <div>
              <label className="block text-sm font-medium mb-2">
                Selecione um funcionário do DP:
              </label>
              <select
                value={selectedDpUserId}
                onChange={(e) => setSelectedDpUserId(e.target.value)}
              ...
              {selectedDpResult && (
                <div className="overflow-x-auto">
                  <h3 className="text-lg font-bold mb-2">
                    Detalhes para: {selectedDpResult.userName}
                  </h3>
                  <table className="min-w-full">
                    <thead>
                      <tr className="border-b dark:border-gray-600">
                        <th className="text-left p-2">Razão Social</th>
                        <th className="text-center p-2">Qtd. Funcionários</th>
                        <th className="text-right p-2">Valor Bonificação</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedDpResult.details.map((detail, i) => (
                        <tr
                          key={i}
                    ...
                    <tfoot>
                      <tr className="font-bold border-t-2 dark:border-gray-500">
                        ...
                        <td className="text-right p-2">
                          {formatCurrency(selectedDpResult.totalBonus)}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
          ...
          {view === "fiscal" && (
            <div>
              <label className="block text-sm font-medium mb-2">
                Selecione um funcionário do Fiscal:
              </label>
              <select
                value={selectedFiscalUserId}
                onChange={(e) => setSelectedFiscalUserId(e.target.value)}
                className="w-full md:w-1/3 p-2 border rounded bg-gray-50 dark:bg-gray-700 dark:border-gray-600 mb-4"
              >
                <option value="">-- Selecione --</option>
              ...
              {selectedFiscalResult && (
                <div className="overflow-x-auto">
                  <h3 className="text-lg font-bold mb-2">
                    Detalhes para: {selectedFiscalResult.userName}
                  </h3>
                  <table className="min-w-full">
                    <thead>
                      <tr className="border-b dark:border-gray-600">
                        <th className="text-left p-2">Razão Social</th>
                        <th className="text-center p-2">Nota Bônus</th>
                        <th className="text-right p-2">Valor Bonificação</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedFiscalResult.details.map((detail, i) => (
                        <tr
                          key={i}
                          className="border-b dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-150"
                        >
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b dark:border-gray-600">
                    ...
                    <th className="text-right p-2">Bonificação Total</th>
                  </tr>
                </thead>
                <tbody>
                  {results.map((result) => (
